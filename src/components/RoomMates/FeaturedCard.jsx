...
const FeaturedCard = ({ mate }) => {
  ...
  return (
    // <div className="rounded overflow-hidden border border-secondary/15 bg-base-100 shadow-md hover:shadow-lg transition duration-300">
    //   <img src={roomPhoto} alt={title} className="w-full h-50 object-cover" />

    //   <div className="p-4">
    //     <div className="flex justify-between items-start">
    //       <h2 className="text-xl font-semibold text-primary">{title}</h2>
    //       <span className={`px-2 py-1 text-xs rounded-full font-medium ${
    //         availability === "not-available" ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"
    //       }`}>
    ...
    //     <p className="mb-2 text-sm text-accent">{description.trim(0,100)}</p>
